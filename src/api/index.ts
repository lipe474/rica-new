...
export async function getArticleById(id: string) {
  const fetchArticle = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/content/article/${id}`,
    {
      cache: "no-store"
    }
  );
...
export async function getArticle() {
  const fetchArticle = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/recentArticles?amount=10&skip`,
    {
...
export async function getRelatedArticles(id: string) {
  const fetchArticle = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/relatedArticles/${id}?amount=2&skip`,
    {
      cache: "no-store"
    }
  );
...
export async function getArticlesByCategory(id: string, amount: number = 2) {
  const fetchArticle = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/category/articles/${id}?amount=${amount}&skip`,
    {
      cache: "no-store"
    }
  );
  ...
  if (!fetchArticle.ok) {
    throw new Error("Failed to fetch  getArticlesByCategory");
  }
